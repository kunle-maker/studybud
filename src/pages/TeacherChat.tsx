  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)] sm:h-[calc(100vh-4rem)] overflow-hidden bg-background">
      ...
      <aside className={`
        ...
        ${sidebarOpen ? "w-60 sm:w-64" : "w-0 overflow-hidden"}
        fixed md:relative h-full md:h-auto
      `}>
        <div className="flex flex-col h-full w-60 sm:w-64">
          {/* Header */}
          <div className="flex items-center gap-2 px-3 py-2.5 sm:py-3 border-b border-border flex-shrink-0">
            <div className="w-7 h-7 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0">
              <i className="fa-solid fa-chalkboard-user text-primary text-xs" />
            </div>
            <span className="text-sm font-semibold text-foreground flex-1 truncate">TeachBuddy</span>
            <button onClick={() => setSidebarOpen(false)}
              className="w-6 h-6 rounded-md flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-muted transition-all md:hidden">
          ...
          <div className="px-3 py-2.5 flex-shrink-0">
            <button onClick={openNewChat}
              className="w-full flex items-center justify-center gap-2 px-3 py-2 sm:py-2.5 rounded-xl text-sm font-semibold text-white bg-primary hover:opacity-90 transition-all">
          ...
          <div className="px-3 pb-2 flex-shrink-0">
            <div className="relative">
              ...
              <input type="text" value={sidebarSearch} onChange={e => setSidebarSearch(e.target.value)}
                ...
                className="w-full pl-8 pr-3 py-1.5 rounded-xl border border-border bg-background text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary/40 transition-all" />
            </div>
          </div>
                ...
                {group.items.map(chat => {
                  ...
                  return (
                    <div key={chat._id} onClick={() => openExistingChat(chat)}
                      className={`group relative flex items-start gap-2.5 px-2 py-2 rounded-xl cursor-pointer transition-all mb-0.5 ${
                      ...
                      <div className={`w-6 h-6 rounded-lg ${s.bg} flex items-center justify-center flex-shrink-0 mt-0.5`}>
                        <i className={`fa-solid ${s.faIcon} ${s.color} text-[10px]`} />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-foreground truncate leading-tight">{chat.title}</p>
                        {chat.preview && (
                          <p className="text-[10px] text-muted-foreground truncate mt-0.5 leading-tight">{chat.preview}</p>
                        )}
                      </div>
                      <button onClick={e => handleDeleteChat(chat._id, e)} disabled={deleting === chat._id}
                        className="opacity-0 group-hover:opacity-100 flex-shrink-0 w-5 h-5 rounded-md flex items-center justify-center text-muted-foreground hover:text-destructive hover:bg-destructive/10 transition-all absolute right-1.5 top-1/2 -translate-y-1/2">
                        {deleting === chat._id
          ...
          <div className="border-t border-border px-3 py-2.5 flex-shrink-0">
            <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/60 mb-2">Style</p>
              ...
              {STYLES.map(s => {
                ...
                return (
                  <button key={s.key} onClick={() => !locked && setStyle(s.key)} disabled={locked}
                    title={locked ? `${s.label} — Premium only` : s.label}
                    className={`relative flex items-center justify-center w-7 h-7 rounded-lg transition-all ${
                      active ? `${s.bg} border border-primary/40`
                        : locked ? "opacity-30 cursor-not-allowed bg-muted"
                          : "bg-muted hover:bg-muted/80 border border-transparent"
                    }`}>
                    <i className={`fa-solid ${s.faIcon} ${active ? s.color : "text-muted-foreground"} text-[10px]`} />
                    {locked && <i className="fa-solid fa-lock text-[7px] text-muted-foreground absolute -top-0.5 -right-0.5" />}
                  </button>
                );
              })}
            </div>
            {!isPremium && (
              <a href="/premium" className="text-[10px] text-primary hover:underline mt-2 block">Upgrade for all styles →</a>
            )}
          </div>
        </div>
      </aside>
